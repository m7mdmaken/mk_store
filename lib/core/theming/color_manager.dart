import 'package:flutter/material.dart';

class ColorsManager {
  static const Color beige = Color(0xFFE9CBA7);
  static const Color softOrange = Color(0xFFF2A679);
  static const Color lightBlue = Color(0xFFA4C7E3);
  static const Color grayBlue = Color(0xFF5B6D7A);
  static const Color sageGreen = Color(0xFF88A47F);
  static const Color offWhite = Color(0xFFF7F3ED);

  static const Color primary = softOrange;
  static const Color secondary = sageGreen;
  static const Color background = offWhite;
  static const Color accent = lightBlue;
  static const Color textDark = grayBlue;
  static const Color textLight = beige;
}
